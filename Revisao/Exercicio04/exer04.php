...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $horas_tarefa = (int) $_POST['horas_tarefa'];
    $complexidade_tarefa = $_POST['complexidade_tarefa'];
    
    $nome_funcionario = $_POST['nome_funcionario'];
    $horas_disponiveis = (int) $_POST['horas_disponiveis'];
    $nivel_experiencia = $_POST['nivel_experiencia'];

    $pode_assumir = true;
    $mensagem = "";

    // Aumenta 10% no tempo necessário da tarefa
    $horas_necessarias = $horas_tarefa * 1.1;

    if ($horas_disponiveis < $horas_necessarias) {
        $pode_assumir = false;
        $mensagem = "O funcionário $nome_funcionario não possui horas suficientes para realizar a tarefa $nome_tarefa.";
    }
